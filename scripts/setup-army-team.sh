...
# 4. Create profiles and copy templates
...
# 5. Create shared kanban board (ignore error if already exists)
echo "[4/5] Creating army kanban board..."
"$HERMES_BIN" kanban boards create army \
  --name "Army Team" \
  --description "Cross-department army team board" \
  --icon 🎖️ \
  --switch \
  --default-workdir "$HOME_DIR/hermes-army/launch-2026" 2>/dev/null || true

# 6. Optional placeholder replacement from env vars
...
for profile in army-hq army-marketing-head army-dev-head army-research-head; do
  ...
  if [[ -n "${TELEGRAM_BOT_TOKEN:-}" ]]; then
    sed -i.bak "s|__TELEGRAM_BOT_TOKEN__|$TELEGRAM_BOT_TOKEN|g" "$profile_dir/.env" 2>/dev/null || true
